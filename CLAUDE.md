...
del plan que salió de ahí. Detalle en
`offsideApp/docs-implementation/frontend-design-system.md` y
`auditoria-frontend-2026-09-09.md`.

⚠️ **La barra superior desbordaba horizontalmente en todo teléfono, en las 21
pantallas.** `header.module.css` no tenía **ni una sola `@media`**: marca de
36px que no encoge + buscador + hasta cinco enlaces con `nowrap`, todo en un
`flex` sin `wrap`. El ancho mínimo de la fila era de ~560px sin sesión y ~670px
con sesión de admin, contra 327px útiles en un teléfono de 375. Ahora se pliega
en un menú con `<details>` —**sin JavaScript**, como el resto del sitio— y el
buscador baja a su propia fila. Verificado en navegador a 375 y a 320px: cero
desborde.

⚠️ **No existía pie de página en ninguna pantalla** (`grep '<footer'` daba
cero). Ahora hay uno, con la cinta de sección del §05 de la identidad. **No
lleva ni un enlace a una pantalla que no exista**, y por eso **no lleva Términos
ni Privacidad**: el alta ya le pide a la gente que los acepte y hoy son texto
plano sin nada que leer. Redactarlos es 🔴 (`legal.md`), así que queda
reportado, no simulado.

⚠️ **Tres colores no llegaban a AA y eran todos texto real.** El naranja de la
...
único componente que está en las 21 pantallas. Ahora el color es una variable
que cada superficie oscura redefine.

⚠️ **La búsqueda no paginaba y el Service ya sabía hacerlo.** `searchListings`
acepta `pagina` y recorta de a 24; la pantalla imprimía el total real y
entregaba 24, **sin ninguna URL que llegara al resto**. Con 38 clubes y 135
temporadas sembradas, el catálogo se cortaba ahí.

**Dos bugs que rompían funcionalidad, encontrados por la auditoría y
arreglados:**

1. ⚠️ **Editar una publicación borraba club, selección, marca, temporada y
   competencia.** El formulario de editar no tiene esos cinco selectores, la
   acción los leía como `undefined` y mandaba `?? null`, y el repositorio
   escribe el `null`: **corregir un typo en el título destruía los catálogos de
   la publicación**. Desaparecía de todas las facetas y perdía el peso `A` de
   los alias (PS-024). No lo avisaba ninguna pantalla y no quedaba registro: los
   catálogos no se auditan. Ahora se pregunta por `formData.has()`, que
   distingue "el campo no vino" de "el vendedor lo vació". ⚠️ **Falta el test
   —es de integración— y faltan los cinco selectores en el formulario.**
2. ⚠️ **Los Server Actions se tragaban `redirect()` y `notFound()`.** En Next
   esas funciones lanzan una excepción de control, los guards de sesión se
   llaman dentro del `try` y el `catch` la convertía en "Tuvimos un problema".
   Con la sesión vencida, quien apretaba Guardar no iba al login: recibía un
   error genérico **sin ninguna salida**. `unstable_rethrow` en
   `mensajeDeError()` cubre los 22 `catch` con cuatro ediciones.

Además: **`metadataBase` y Open Graph** —la ficha compartida por WhatsApp
llegaba como un enlace pelado, sin título ni foto—, favicon (no había),
`loading.tsx` en la raíz y en los cuatro grupos (no había ninguno, con todo el
sitio en `force-dynamic`), enlace de saltar al contenido, áreas táctiles de
44px, buscador a 16px (abajo de eso iOS hace zoom y no lo deshace), pastillas de
filtro activo con "Limpiar todo", filtros plegables en teléfono, y la ficha
reordenada para que en móvil el precio no quede debajo de ocho fotos.

⚠️ **Falta pasar las pantallas por un navegador con la app corriendo.** En la
máquina donde se hizo el trabajo no hay Docker, así que no hay PostgreSQL ni
Redis. El CSS se verificó contra un banco de pruebas armado con los
`*.module.css` reales —ahí apareció que **el botón primario era invisible sobre
la portada verde**—, y `format`, `lint`, `typecheck` y los **281 tests
unitarios** están en verde. **Los 257 de integración no se corrieron.**

⚠️ **Las fases 4 y 5 quedaron sin hacer a propósito** (ver la auditoría). Lo más
urgente de lo que queda: el retorno `?status=pending` de Mercado Pago no está
contemplado, "Mis compras" no dice qué compraste, y **el ciclo de vida de la
orden se corta en `PAID`** —DEC-029 fija siete estados y existen dos—.

**Frontend completo de punta a punta — ✅ (2026-09-10)**: las 22 pantallas
rediseñadas, capa de movimiento propia y once primitivas nuevas. **71
archivos.** Detalle en `offsideApp/docs-implementation/frontend-2026-09-10.md` y
`frontend-movimiento.md`.

**Movimiento**: tokens de duración con rol, entradas al primer pintado,
**revelado por scroll en CSS puro** (`animation-timeline: view()`, sin
IntersectionObserver ni hidratación) y **transiciones entre pantallas con la View
Transitions API de React 19** —Next 16 las soporta sin configuración—, incluido
el **morph de la foto del producto entre la vitrina y la ficha**. La barra y el
pie quedan anclados para que haya un punto fijo.

⚠️ **Hay una tabla explícita de QUÉ NO SE ANIMA, y es parte del contrato**: el
precio, el total, los avisos de error, el estado de una orden y la consola de
pagos. Un número que se mueve se lee como un número que todavía no está
decidido, y esto es un sitio donde se transfiere plata.

⚠️ **`prefers-reduced-motion` se apaga en DOS capas.** La regla global usa el
selector `*`, que **no alcanza a los pseudo-elementos del árbol de transición**;
y una animación dirigida por scroll **no se apaga bajándole la duración**,
porque su progreso lo da el scroll: por eso la regla entera vive adentro de la
media query. Sin eso, quien pide menos movimiento se queda mirando contenido en
`opacity: 0`.

**Errores de formulario por campo**: `lib/formulario.ts` + `lib/errores.ts`.
⚠️ **El formulario ya no se vacía al fallar**: un código postal de tres dígitos
borraba las seis líneas de dirección tipeadas en un teléfono, justo después de
decidir comprar. Y el error dice de qué campo es. ⚠️ Las contraseñas nunca
vuelven al cliente. ⚠️ El plazo del rate limit **se calculaba y se tiraba**: la
pantalla decía "unos minutos" teniendo el TTL exacto de Redis.

**Tres arreglos que rompían cosas:**

1. ⚠️ **`?status=pending` de Mercado Pago no estaba contemplado y es
   alcanzable**: la preferencia no excluye efectivo, así que quien paga por
   Rapipago vuelve así y no veía **ningún** mensaje, sólo un botón de pagar que
   invitaba a pagar dos veces. Ahora además la pantalla **se actualiza sola
   mientras se confirma, sin JavaScript** —un `<meta http-equiv="refresh">` con
   el contador en la URL, que se detiene solo— y el botón de pagar se esconde
   mientras tanto.
2. ⚠️ **Faltaba `ORDER BY` en tres consultas.** Sin él PostgreSQL reordena las
...
**Los cinco selectores de catálogo llegaron al formulario de editar**: es la otra
mitad del bug del 2026-09-09. El parche impedía **borrar** club, marca y
temporada al corregir un typo; esto permite **editarlas**.

**Mercado Pago pasó de dos estados a cuatro**: `expired` —que el barrido diario
marca cuando MP rechaza renovar— y `revoked` se mostraban igual que "nunca
conectada", en la pantalla que existe para gestionarlas. Y se muestra
`mpUserId`, el único dato con el que alguien con dos cuentas puede saber **cuál**
vinculó.

⚠️ **El índice del back-office prometía que TODO queda en el log de auditoría, y
es falso**: `setCommissionRateBasisPoints` no escribe en `audit_log` —su propio
comentario explica por qué—. Los reembolsos sí. Ahora el texto dice los dos
mecanismos. Además la consola **muestra `mpPaymentId`**, que era el dato más
importante de la pantalla y estaba oculto mientras el aviso pedía "verificá en
Mercado Pago".

**Pantalla nueva `/como-funciona`.** ⚠️ Su sección más importante es **"Lo que
todavía no hacemos"** —sin envíos, sin reseñas, sin reclamos, autenticidad
declarada por el vendedor—: un marketplace nuevo que promete todo se parece a
una estafa. Ninguna cifra está escrita a mano: la comisión sale del Config Store.

Además: **44 mensajes de error reescritos** (tildes, voseo, y fuera los códigos
internos que se filtraban al texto visible), un **`error.tsx` por grupo de
rutas** —con uno solo en la raíz, un fallo en la compra se llevaba puesta la
barra—, "Última unidad" en las dos grillas, historial de la comisión, ventas
como tabla comparable, el formulario de publicar en cuatro grupos, y
confirmación en dos pasos —con `<details>`, **no** `window.confirm`— para
eliminar, desvincular y reembolsar.

⚠️ **Sigue faltando pasar las pantallas por un navegador con la app corriendo**:
no hay Docker en la máquina donde se trabajó. El CSS se verificó contra un banco
de pruebas armado con los `*.module.css` reales a 1280, 760 y 375px —ahí
aparecieron dos de los defectos de arriba—, y `format`, `lint`, `typecheck` y
los **281 tests unitarios** están en verde. **Los 257 de integración no se
corrieron.**

⚠️ **Quedó fuera por tamaño, no por bloqueo**: favoritos (`favorites` está
migrada y el icono dibujado), perfil público del vendedor (SS-020 ✅), el filtro
...
`offsideApp/docs-implementation/frontend-rediseno-2026-09-10.md`.

⚠️ **ESTABA MIRANDO EL BUILD DEL 2026-09-02.** Verificado en el navegador contra
`offside.com.ar`: `body` sin `display: flex`, sin `<footer>`, sin botones en la
portada, sin la tira de garantías. **Nunca vio el trabajo del 09 ni del 10.**
Eso explica parte del diagnóstico y no lo invalida: lo que había era correcto y
austero, y austero era el problema. **Desplegar cierra la mitad del reclamo.**

**Dos ideas sostienen la fundación**: **un eje de 112°** —el brazo del juez de
línea con la bandera levantada— que usan todos los gradientes, rayas, destellos
y barridos, de modo que veinte efectos se leen como un sistema; y **cuatro
planos** (fosa, papel, ficha, flotante) con elevación de seis niveles teñida en
Tinta, grano y mallas. Antes había **dos** superficies —papel y blanco separadas
por un borde gris de 1px— y por eso nada estaba delante ni detrás de nada.

⚠️ **LA LUZ SE RESTA, NUNCA SE SUMA, y es lo que sostiene todos los contrastes
del sitio.** Papel sobre Verde Cancha da 5.16:1 contra un mínimo AA de 4.5: hay
0.66 de aire. Está medido que aclarar ese verde apenas un **7%** con blanco ya
rompe AA. Por eso el punto más claro de cualquier malla de marca **es el color
de marca plano**. Con esa regla los contrastes valen **por construcción**, en
cualquier viewport, sin volver a medir.

⚠️ **Las superficies son un CONTRATO** (`.sup-noche`, `.sup-cancha`, `.sup-2`…):
ningún módulo escribe un fondo oscuro, un vidrio ni un grano a mano. La causa
medida de que todo se viera plano eran **35 declaraciones de
`background: var(--color-blanco)` en 11 archivos**. Y **ninguna superficie
oscura redefine `--color-tinta` ni `--color-blanco`**: es el atajo obvio y es
una mina —hay tres bloques con fondo Tinta que quedarían papel sobre papel—.

**Movimiento**: de 6 keyframes a un vocabulario completo —marquesinas, paralaje
por capas, destello diagonal, subrayado que se dibuja, revelado tipográfico por
líneas, barra que se despega— **todo en CSS puro, sin una línea de JavaScript**.

⚠️ **Cuatro reglas del movimiento que se pagan caras si se olvidan**: las **dos
guardas** (`@supports` por afuera —sin él la página queda **VACÍA**, no "sin
animar"— y `prefers-reduced-motion` por adentro); **toda animación infinita
adentro de la media query**, nunca confiando en la regla global, que aplasta la
duración y la deja clavada en el **último** cuadro; **`animation-delay` no hace
nada en una animación dirigida por scroll**, el escalonado se corre con
`animation-range`; y **nada de `box-shadow` en una línea de tiempo de scroll**,
que es pintura y la barra adherida es el escenario más caliente del sitio.

**Cinco cosas que estaban rotas y nadie veía:**

1. ⚠️ **El patrón de rombos de la marca NUNCA cambió de color, en todo el
   sitio.** `--patron-rombos` se declaraba en `:root` con `var(--patron-color)`
   adentro, y **el valor computado de una custom property lleva sus `var()` ya
   sustituidos**: se resolvía ahí con el gris y los descendientes heredaban eso.
   Las redefiniciones de `.sup-noche` y `.sup-cancha` no cambiaban una pieza.
   Viene del 2026-09-09.
2. ⚠️ **La barra de progreso de lectura no pintaba nunca**: `barrer-x` declaraba
   sólo `from { scaleX(0) }` y el elemento también, así que interpolaba **0→0**.
3. ⚠️ **Los acentos de los titulares se rapaban.** El recorte por línea
   compensaba 0.06em y la tinta de `á`/`é` en Big Noodle se va **0.1029em** —
   medido en el navegador con la Canvas API sobre la woff2 real—. O sea que
   **toda vocal acentuada perdía el acento**: se leía "TENES UNA CAMISETA".
4. ⚠️ **El borde de TODOS los campos de formulario daba 1.26:1**, contra el 3:1
   que WCAG 1.4.11 exige para el contorno de un control.
5. ⚠️ **Un desborde horizontal real a 320 y 375px** en la vitrina: `.boton` lleva
   `white-space: nowrap` a propósito, y un elemento flex no puede encogerse por
   debajo de su min-content — con `nowrap` el min-content **es** el texto entero,
   así que la etiqueta no se recorta, **empuja el `<body>`**.

⚠️ **Un hecho de negocio FALSO en la pantalla más visitada**: la vitrina decía
"Offside no toca la plata". Offside **sí** toca la plata —la comisión sale del
mismo pago, DEC-043—, y eso se lee como "no cobra comisión". Dice **"Offside no
retiene los fondos"**, que es lo único que puede prometer y es lo que DEC-019
cerró en negativo.

**`scripts/chequeo-css.mjs`, enganchado a `verify`**: verifica cuatro
invariantes que **fallan en silencio** —`var()` inexistente, `@keyframes`
pisado entre un global y un módulo, y un **`estilos.X` que la hoja no define**,
que TypeScript NO ve porque los tipos de un CSS Module son un diccionario
abierto—. Se ganó el lugar en el acto: la sesión se cortó por límite de uso a
mitad de una ola y dejó `vendedor.module.css` reescrito con seis pantallas sin
migrar; el chequeo listó las nueve referencias rotas y el typecheck daba verde.

⚠️ **ESTA VEZ SÍ SE VIO EN UN NAVEGADOR**, que era el pendiente de las dos
sesiones anteriores. Sigue sin haber Docker, así que se armó un banco que
**renderiza los componentes reales** —`app/page.tsx` es un Server Component
async y se lo llama como función, con stubs en lugar de los Services— servido
por HTTP. Medido ahí a 1280/375/320: **cero desborde, cero controles chicos,
cero elementos trabados en `opacity: 0`**. Y las dos variantes que pueden dejar
la página vacía en vez de quieta: con `prefers-reduced-motion` → 0 invisibles y
**0 animaciones corriendo**; sin `animation-timeline` (Firefox hoy) → 0
invisibles y documento completo.

⚠️ **Lo que el banco NO cubre**: las View Transitions entre pantallas, el morph
de la foto de la grilla a la ficha, y **19 de las 22 pantallas** (se renderizaron
