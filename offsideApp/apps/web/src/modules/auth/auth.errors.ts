...
export type AuthErrorCode =
  ...
  | 'LAST_IMAGE_REQUIRED'
  | 'LISTING_DELETED'
  | 'LISTING_NOT_PAUSABLE'
  | 'LISTING_NOT_RESUMABLE'
  | 'IMAGE_REQUIRED'
...
export const emailAlreadyRegistered = (): AuthError =>
  new AuthError('EMAIL_ALREADY_REGISTERED', 'Ese email ya tiene una cuenta. Probá ingresando');

/** BR-001: no se opera sin email verificado. */
export const emailNotVerified = (): AuthError =>
  new AuthError(
    'EMAIL_NOT_VERIFIED',
    'Verificá tu email antes de comprar o vender. Te mandamos un enlace al registrarte',
  );

export const accountNotActive = (): AuthError =>
  new AuthError('ACCOUNT_NOT_ACTIVE', 'Esta cuenta no está activa');

export const invalidToken = (): AuthError =>
  new AuthError('INVALID_TOKEN', 'Ese enlace no sirve o ya venció. Pedí uno nuevo');

export const sessionInvalid = (): AuthError =>
  new AuthError('SESSION_INVALID', 'Tu sesión venció. Ingresá de nuevo');

export const notAuthenticated = (): AuthError =>
  new AuthError('NOT_AUTHENTICATED', 'Necesitás iniciar sesión');

export const forbidden = (): AuthError =>
  new AuthError('FORBIDDEN', 'No tenés permiso para hacer esto');

export const sellerProfileAlreadyExists = (): AuthError =>
  new AuthError('SELLER_PROFILE_ALREADY_EXISTS', 'Esta cuenta ya tiene un perfil de vendedor');
