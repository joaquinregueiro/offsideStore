 ...
 */
async function leerReclamo(id: string, volverA: string): Promise<DisputeDetail> {
  const user = await requireVerifiedSessionUser(volverA);

  try {
    return await getDispute(user, id);
  } catch (error) {
...
export default async function FichaDeReclamo({ params }: { params: Promise<{ id: string }> }) {
  ...
  const reclamo = await leerReclamo(id, `/cuenta/reclamos/${id}`);
            ...
            <section className={`${estilos.bloque} sup-ficha`}>
              ...
              {reclamo.resolution === null ? (
                <p className={estilos.nota}>
                  Todavía no hay resolución. Cuando Offside revise el reclamo, la decisión y lo que
                  implique van a aparecer acá y en la conversación.
                </p>
              ) : (
                <>
